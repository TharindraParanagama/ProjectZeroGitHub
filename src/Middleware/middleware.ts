...
export const sess:any=session({secret:'test',saveUninitialized:true,resave:true})
...
export let auth:any = function(req:any, res:any,next:any) {
   ...
   const pass:any=bcrypt.hashSync(req.session.password, saltRounds)
    ...
    db.one('SELECT username,password FROM members WHERE username=${user_name} AND password=${password} AND role=${role}',{
